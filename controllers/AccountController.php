...
{
	protected $auth_actions = array('index', 'signout', 'follow', 'unfollow');
	...
	public function registerAction()
	{
		...
		if(!strlen($user_name)) {
			...
			$errors[] = 'ユーザIDは半角英数字およびアンダースコアを3～20文字以内で入力して下さい';
	...
	public function authenticateAction()
	{
		...
		if(count($errors) === 0) {
			...
			if(!$user || !$user_repository->verifyPassword($password, $user['password'])) {
	...
	public function followAction()
	{
		...
		$token = $this->request->getPost('_token1');
	...
	public function unfollowAction()
	{
		...
		$token = $this->request->getPost('_token2');
		if(!$this->checkCsrfToken('account/unfollow', $token)) {
			return $this->redirect('/user/' . $following_name);
		}
